...
.. note::
   This demo dataset is **RNA002**, **transcriptome-aligned** data from the original xPore paper. It predates genome alignment support — for the full raw-data-to-diffmod pipeline, including genome alignment, see :ref:`Quickstart <quickstart>`.
...
Note that the FAST5, FASTQ and BAM files are required to obtain the eventalign file with Nanopolish, xPore only requires the eventalign file. This demo already provides the eventalign file, so it skips the basecalling, alignment and resquiggling steps — see the :ref:`Quickstart <quickstart>` page for how to produce an eventalign file from raw reads.
