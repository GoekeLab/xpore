"""
Copied from https://github.com/python/cpython/blob/3.8/Lib/statistics.py.
"""
...
from math import hypot, sqrt, fabs, exp, erf, log, fsum #, tau
...
class NormalDist:
    ...
    def overlap(self, other):
        ...
        ## My code # prefer cdf to pdf because pdf is not bounded.
        # is_not_inside = ((Y.cdf(x1) < 0.01) and (X.cdf(x1) < 0.01)) or ((Y.cdf(x2) < 0.01) and (X.cdf(x2) < 0.01)) or (( (1-Y.cdf(x1)) < 0.01) and ((1-X.cdf(x1)) < 0.01)) or (( (1-Y.cdf(x2)) < 0.01) and ((1-X.cdf(x2)) < 0.01))
        # print(x1,x2,X.cdf(x2),Y.cdf(x1))
        list_cdf_at_intersections = [X.cdf(x1),Y.cdf(x1),X.cdf(x2),Y.cdf(x2)]
        ##
        return 1.0 - (fabs(Y.cdf(x1) - X.cdf(x1)) + fabs(Y.cdf(x2) - X.cdf(x2))),list_cdf_at_intersections
